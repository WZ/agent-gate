...
|---|---|---|---|
| **Claude Code** (`claude`) | Yes, primary path | <ul><li>Auto-detected; `api.anthropic.com` seeded</li><li>Anthropic Messages: SSE, tool calls/results, system prompts, tokens</li></ul> | <ul><li>✅ None for the local CLI</li><li>Bedrock / Vertex when fixtures land</li></ul> |
| **Codex** (`codex`) | Capture yes (via HTTP fallback) | <ul><li>Auto-detected; `chatgpt.com` + `api.openai.com` seeded</li><li>OpenAI Responses parser on `POST /backend-api/codex/responses` (zstd in, SSE out)</li><li>`chatgpt.com/backend-api/...` setup endpoints parsed</li><li>WS transport pins TLS → empty 101 + `ws_pinned_upstream`</li></ul> | <ul><li>✅ None — HTTP fallback covers the model conversation</li><li>WS hijack ships as infrastructure for future non-pinning agents</li></ul> |
| **Aider** (`aider`) | Capture yes | <ul><li>Auto-detected; `api.anthropic.com` + `api.openai.com` seeded</li><li>Anthropic Messages + OpenAI Chat / Responses parsed</li><li>Other providers: generic HTTP</li></ul> | <ul><li>More providers as fixtures land</li></ul> |
| **OpenCode** (`opencode`) | Capture yes | <ul><li>Auto-detected; `api.anthropic.com` seeded</li><li>Anthropic Messages + OpenAI Chat / Responses parsed</li><li>Other providers may need manual trust → generic HTTP</li></ul> | <ul><li>Plan 5's parser registry — new decoders are one-file additions</li></ul> |
| **OpenClaw / Hermes Agent** | Manual capture; not first-class today | <ul><li>Run via `agent-gate run -- <cmd>` or `HTTPS_PROXY`</li><li>Anthropic + OpenAI HTTP parsed; others generic</li></ul> | <ul><li>Binary detection</li><li>Provider host seeding</li><li>More vendor fixtures</li></ul> |
| **curl, scripts, MCP clients, custom agents** | Capture yes | <ul><li>Any HTTPS client via airtight or `HTTPS_PROXY`</li><li>Anthropic + OpenAI HTTP parsed; others generic</li></ul> | <ul><li>First-class decoders as fixtures are captured</li></ul> |
