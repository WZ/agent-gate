...
- **Plan 5 (`v0.5.0-multi-vendor`)** — first-class parser branches for OpenAI Chat Completions, OpenAI Responses, Anthropic Bedrock, Vertex `generateContent`, and deeper Codex/ChatGPT backend traffic. Codex, Aider, OpenCode, and anything else that talks HTTP already work via capture + generic parsing; Plan 5 makes more of their dashboard views as rich as Anthropic's.
